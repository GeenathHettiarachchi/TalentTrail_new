...
const DeveloperTable = React.memo(({
  ...
  onEdit,
  onDelete,
  onAssignLead,     // Function to assign a developer lead
  currentLeadId,    // Current lead intern ID for developers
  isLoading = false
}) => {
  ...
  const handleRowClick = useCallback((intern, e) => {
    ...
    const languages = toList(intern.languagesAndFrameworks);
    const projects = toList(intern.projects);
    if (languages.length > 2 || projects.length > 2) {
        ...
        <table className={styles.table}>
          ...
          <tbody className={styles.tbody}>
            {interns.map((intern) => {
              ...
              const langHidden = Math.max(0, languages.length - 2);
                  ...
                  >
                    ...
                    <td className={styles.td}>
                      {languages.length === 0 ? (
                        ...
                        <div className={styles.cellPills} aria-label="Languages and frameworks">
                          {languages.slice(0, 2).map((lang, idx) => (
                            <span key={idx} className={styles.projectBadge}>{lang}</span>
                          ))}
                        </div>
                      )}
                    </td>
                      ...
                      {projects.length === 0 ? (
                        ...
                        <div className={styles.cellPills} aria-label="Projects">
                          {projects.slice(0, 2).map((proj, idx) => (
                            <span key={idx} className={styles.projectBadge}>{proj}</span>
                          ))}
                        </div>
                      )}
                    </td>
                        ...
                        <div className={styles.expandedContent}>
                          ...
                          {projects.length > 2 && (
                            <div className={styles.expandedSection}>
                              ...
                              <div className={styles.projectsList}>
                                {projects.map((proj, i) => (
                                  <span key={i} className={styles.projectBadge}>{proj}</span>
                                ))}
                              </div>
                            </div>
                          )}
