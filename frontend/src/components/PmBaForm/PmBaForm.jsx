...
const PmBaForm = ({ isOpen, onClose, onSubmit, editingIntern = null, isLoading = false }) => {
  ...
  useEffect(() => {
    if (editingIntern) {
      ...
      setFormData({
        ...
        trainingEndDate: editingIntern.trainingEndDate ? editingIntern.trainingEndDate.split('T')[0] : '',
  ...
  const validateForm = () => {
    ...
    else if (formData.internCode.length < 3) newErrors.internCode = 'Intern code must be at least 3 characters';
    ...
    if (formData.mobileNumber.trim() && !/^(\+?\d{9,15}|0\d{9})$/.test(formData.mobileNumber.trim())) {
      newErrors.mobileNumber = 'Enter a valid phone number';
    }

    if (!formData.trainingEndDate) newErrors.trainingEndDate = 'Training end date is required';
    else {
      const endDate = new Date(formData.trainingEndDate);
      const today = new Date();
      today.setHours(0,0,0,0);
      if (endDate < today) newErrors.trainingEndDate = 'End date cannot be in the past';
    }
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ ...formData, internId: editingIntern?.internId ?? null });
  };
      ...
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <div className={styles.formActions}>
            <button type="button" className={styles.cancelButton} onClick={handleClose} disabled={isLoading}>Cancel</button>
            <button type="submit" className={styles.submitButton} disabled={isLoading}>
              {isLoading ? (<> <div className={styles.spinner}></div> {editingIntern ? 'Updating...' : 'Adding...'} </>) : (editingIntern ? 'Update Intern' : 'Add Intern')}
            </button>
          </div>
        </form>
      </div>
